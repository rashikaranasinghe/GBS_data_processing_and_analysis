...
# Originally developed by Joana Meier. modifications added by Rashika Ranasinghe
...
plink_1.9 --file $file --make-bed --out $file --allow-no-sex --allow-extra-chr 0
plink_1.9 --bfile $file --freq --missing --within $file".clust" --out $file --allow-no-sex --allow-extra-chr 0
...
awk 'BEGIN{scaffold="";pos=0;newpos=0}
	{if($2==scaffold){newpos=pos+$3}else{scaffold=$2;pos=newpos};chpos=pos+$3;print $0,chpos}' $file".frequencies" > $file".frequencies.newpos"
